...
第5.2节[图1](#q1-coverage-comparison)给出相同36米直径的对照：正方形的直径圆半径为18米，恰好覆盖全域；等边三角形的直径圆半径也为18米，却小于其最小包围圆半径 \(12\sqrt3\) 米，因而不能覆盖。图中实线表示各自的最小包围圆。
...
例如，[图1](#q1-coverage-comparison)中两区域均满足 \(D=36<40\) 米，但正方形的最小半径为18米，等边三角形为 \(12\sqrt3\approx20.784610\) 米，20米圆覆盖结论相反。因此 \(D\le40\) 是必要条件，不能作为充分条件；二者都落在需要进一步求圆的Jung预判区间。
...
具体的局部修正以逐坐标取最近网格点所得的 \(C_0\) 为起点，半格处任选一种固定规则。将其本身与8个邻点组成候选集，并剔除超出接口坐标范围的点：

\[
\begin{aligned}
\mathcal N_h(C_0)&=\{C_0+h(a,b):a,b\in\{-1,0,1\}\}\cap\mathcal G_h,\\
\mathcal V_h(C_0)&=\{C\in\mathcal N_h(C_0):R_{Q,U}(C)+e\le20\},\\
\text{局部校验结果}&=
\begin{cases}
\mathtt{CLEAR\_READY},&\mathcal V_h(C_0)\ne\varnothing,\\
\mathtt{COVERAGE\_UNCERTAIN},&\mathcal V_h(C_0)=\varnothing\text{ 且尚无全局不可行证明}.
\end{cases}
\end{aligned}
\]

本式以已确认未清除源、区域非空且数值界一致为前提。有可行邻点时选定一个并发送其已核验坐标；当前邻域无已认证可行点时继续扩大搜索或细化数值界。八邻域只是快速修正范围，不是全部可行中心的搜索范围，第5.5节给出九个局部点全部不可行、较远网格点却可行的反例。
...
覆盖检验求的是到指定圆心 \(Z\) 的最大距离，与支持函数的投影极值不同。设一段圆弧为 \(\Gamma=\{C+r\,u(t):t\in[a,b]\}\)，则

\[
\|C+r\,u(t)-Z\|^2
=\|C-Z\|^2+r^2+2r(C-Z)^{\mathsf T}u(t).
\]

当 \(C\ne Z\) 时，最后一项在 \(u(t)\) 与 \(C-Z\) 同向时最大。因此只需检查弧端点与下列径向候选点：

\[
\begin{aligned}
X_{\mathrm{far}}&=C+r\frac{C-Z}{\|C-Z\|},\qquad(C\ne Z),\\
\mathcal V_\Gamma(Z)&=\{C+r\,u(a),C+r\,u(b)\}\ \cup
\begin{cases}
\{X_{\mathrm{far}}\},&C\ne Z\text{ 且 }X_{\mathrm{far}}\in\Gamma,\\
\varnothing,&\text{其他情形},
\end{cases}\\
\max_{X\in\Gamma}\|X-Z\|&=\max_{X\in\mathcal V_\Gamma(Z)}\|X-Z\|.
\end{aligned}
\]

弧内条件按第1.1节的连续扫角规则判断；若 \(C=Z\)，整段弧距离恒为 \(r\)，端点即可代表。此处的候选是沿圆心连线背离 \(Z\) 的径向点，不是切点。将全部线段端点及每条弧的候选距离取最大，即得到 \(R_K(Z)\)，无需对固定圆心的弧上距离另做采样搜索。
...
这里的 \(\varepsilon_r\) 控制覆盖半径与最优半径的差，并不直接保证圆心坐标误差。更小容差需相应收紧计算误差界；有限终止证明本身既不提供微米级数值认证，也不说明实际通常需要多少轮增补，实际轮数须由完整实现记录。
...
八邻域搜索的局限可由另一条线段说明：端点为 \((-7.4234,-18.5681)\)、\((7.4314,18.5689)\)，连续最优中心为 \((0.004,0.0004)\)，最小半径约19.998887米。以原点为中心、间距0.01米的九个局部网格点，其最小覆盖半径仍约20.000744米；但邻域外的 \((-0.02,0.01)\) 仅需约19.998903米即可覆盖。所有距离平方采用精确有理数比较，故局部失败不能触发全局 `COVERAGE_FAIL`。

圆弧最远点例取圆心为原点、半径5米、待检验中心 \(Z=(10,0)\)。左半圆弧包含径向点 \((-5,0)\)，最远距离为15米，超过两端点距离 \(\sqrt{125}\) 米；右半圆弧不包含该径向点，最远点改为端点。若待检验中心与圆心重合，全部弧点的距离均为5米。解析候选与独立弧采样结果一致，采样只用于核验例子，不替代全域极值公式。
